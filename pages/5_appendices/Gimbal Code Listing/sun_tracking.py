"""
sun_tracker.py — real-time Sun tracker using PCHIP Hermite + smart flip (hysteresis)
...
UPDATE_INTERVAL = 5.0        # seconds between major updates (knot spacing) — Sun moves slowly
INTERP_STEPS    = 6          # sub-steps within UPDATE_INTERVAL (sends per UPDATE_INTERVAL)
ELEV_CUTOFF_DEG = 5.0        # ignore targets below this elevation (deg)
...
# Frame/limits
AZ_ZERO_DEG     = 0.0        # 0° = North (later: replace with dual-antenna true-north offset)
...
# Ephemeris file for Sun position
# Note: Skyfield will download this the first time if not present.
...
def map_to_gimbal_frame_with_state(
    az_sky: float,
    el_sky: float,
    az_zero: float,
    flipped_state: bool
...
def main():
    ...
    # Load ephemeris + bodies (Sun tracking)
    try:
        eph = load(EPHEMERIS_FILE)
    except Exception as e:
        raise RuntimeError(
            f"[EPH] Failed to load ephemeris '{EPHEMERIS_FILE}'. "
            f"If this is your first run, ensure your PC can download it once. Details: {e}"
        )
    sun = eph["sun"]
    earth = eph["earth"]
    observer = earth + gs

    print(f"[INFO] Tracking: {TARGET_NAME}")
    ...
    print(
        f"[CFG] GS=({GS_LAT:.6f},{GS_LON:.6f},{GS_ALT_M:.1f}m)  step={UPDATE_INTERVAL}s  "
        f"interp={INTERP_STEPS}  mode=PCHIP  AZ_zero={AZ_ZERO_DEG}°  dry={DRY_RUN}  cutoff={ELEV_CUTOFF_DEG}°"
    )
    ...
    try:
        while True:
            ...
            # Sample at knots (Sun)
            alt0, az0, _ = observer.at(t0).observe(sun).apparent().altaz()
