...
def main():
    ...
    if not DRY_RUN:
        ...
        # ===== MODIFICATION (MINIMAL): set AC/DC once at start =====
        az_acc_counts = int(4.0 * gimbal.cnt_az)   # 4 deg/s^2
        el_acc_counts = int(4.0 * gimbal.cnt_el)
        az_dec_counts = int(6.0 * gimbal.cnt_az)   # 6 deg/s^2
        el_dec_counts = int(6.0 * gimbal.cnt_el)

        gimbal._cmd(f"AC {az_acc_counts},{el_acc_counts}", quiet=True)
        gimbal._cmd(f"DC {az_dec_counts},{el_dec_counts}", quiet=True)
        # =========================================================
    ...
    # ===== MODIFICATION (MINIMAL): first move uses wait=True =====
    first_move = True
    # ===========================================================
        ...
        while True:
            ...
            for i in range(INTERP_STEPS + 1):
                ...
                if DRY_RUN:
                    ...
                    try:
                        # ===== MODIFICATION (MINIMAL): ramp only on first move =====
                        if first_move:
                            gimbal.degSteer(az_gim, el_gim, absolute=True, wait=True)
                            first_move = False
                        else:
